...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="Item[@type='Citect.Ampla.Plant2Business.Server.Database2AmplaIntegration']" mode='include'>
		<xsl:text>Yes</xsl:text>
	</xsl:template>
	
	<xsl:template match="Item[@type='Citect.Ampla.Plant2Business.Server.Database2AmplaIntegration']" mode='get-png-filename'>
		<xsl:param name='name' select='@hash'/>
		<xsl:value-of select="concat('d2a_', $name, '.png')"/>
	</xsl:template>
	
	<xsl:template match="Item[@type='Citect.Ampla.Plant2Business.Server.Database2AmplaIntegration']" mode='graph'>
		...
		<xsl:variable name="source-db">Source</xsl:variable>	
		<!--
		...
		<dotml:graph file-name="{$filename}" label="Database2Ampla: {@fullName}" rankdir="LR" fontname="Arial" fontsize="12.0" labelloc='t' size="8,8">
			<dotml:cluster id="{concat(@hash, '_source')}" label="{concat('Source\n', $source-db)}" style="solid" labelloc="t" >
				<dotml:record>
